...
class WindowedSpectrogramPlugin extends BasePlugin {
    ...
    async redrawSegmentCanvas(segment) {
        if (!segment.canvas)
            return;
    ...
    async renderChannelToCanvas(channelFreq, ctx, width, height, yOffset, freqFrom, freqMin, freqMax) {
        ...
        const bitmap = await createImageBitmap(imageData, 0, Math.round(freqBins * (1 - rMax1)), channelFreq.length, Math.round(freqBins * (rMax1 - rMin)));
    ...
    cleanupOldSegments(windowStartTime, windowEndTime) {
